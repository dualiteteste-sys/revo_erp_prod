...
import { listVendas, VendaPedido } from '@/services/vendas';
...
export default function PedidosVendasPage() {
  ...
  const fetchOrders = async () => {
    setLoading(true);
    try {
      const data = await listVendas(debouncedSearch, statusFilter);
      setOrders(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <div className="p-1">
      ...
      <Modal isOpen={isFormOpen} onClose={handleClose} title={selectedId ? 'Editar Pedido de Venda' : 'Novo Pedido de Venda'} size="5xl">
