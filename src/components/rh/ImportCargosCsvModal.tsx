...
import type { CargoPayload } from '@/services/rh';
import { getFirst, parseBoolPt, parseCsv, type ParsedCsvRow } from '@/lib/csvImport';
...
type PreviewRow = {
  ...
  ativo: boolean;
  errors: string[];
  payload: CargoPayload | null;
};

type WizardStep = 0 | 1 | 2;

type TargetFieldKey = 'nome' | 'descricao' | 'responsabilidades' | 'autoridades' | 'setor' | 'ativo';
type FieldMapping = ImportFieldMapping<TargetFieldKey>;

type DedupeKey = 'nome';
type DedupeStrategy = 'none' | 'first' | 'last';

const MAPPING_STORAGE_KEY = 'revo:import_mapping:cargos:v1';

const TARGET_KEYS: TargetFieldKey[] = ['nome', 'descricao', 'responsabilidades', 'autoridades', 'setor', 'ativo'];

const TARGET_FIELDS: Array<{ key: TargetFieldKey; label: string; required?: boolean }> = [
  { key: 'nome', label: 'Nome do Cargo', required: true },
...
const FIELD_SYNONYMS: Record<TargetFieldKey, string[]> = {
  nome: ['nome', 'cargo', 'titulo', 'title', 'name', 'funcao', 'função'],
  descricao: ['descricao', 'descrição', 'description'],
  responsabilidades: ['responsabilidades', 'responsabilidade', 'atribuicoes', 'atribuições'],
  autoridades: ['autoridades', 'autoridade', 'poderes'],
  setor: ['setor', 'departamento', 'area', 'área', 'department'],
  ativo: ['ativo', 'status', 'active'],
};

export default function ImportCargosCsvModal(props: {
  isOpen: boolean;
  onClose: () => void;
  onImported: (summary: { ok: number; failed: number }) => void;
  importFn: (payload: CargoPayload) => Promise<any>;
}) {
  ...
  const [previewSort, setPreviewSort] = useState<SortState<'line' | 'nome' | 'setor' | 'ativo' | 'errors'>>({
    column: 'line',
    direction: 'asc',
  });
  ...
  function buildPreviewRow(r: ParsedCsvRow): PreviewRow {
    ...
    const descricao = maybeUpper(resolveField(row, 'descricao'));
    const responsabilidades = maybeUpper(resolveField(row, 'responsabilidades'));
    const autoridades = maybeUpper(resolveField(row, 'autoridades'));
    const setor = maybeUpper(resolveField(row, 'setor'));
    const ativoRaw = resolveField(row, 'ativo');

    if (!nome) errors.push('nome é obrigatório');

    const ativo = ativoRaw ? (parseBoolPt(ativoRaw) ?? true) : true;

    const payload: CargoPayload | null =
      errors.length > 0
        ? null
        : {
            nome: nome.trim(),
            descricao: descricao ? descricao.trim() : null,
            responsabilidades: responsabilidades ? responsabilidades.trim() : null,
            autoridades: autoridades ? autoridades.trim() : null,
            setor: setor ? setor.trim() : null,
            ativo,
          };

    return { line: r.line, nome, setor: setor || null, ativo, errors, payload };
  }

  const { preview, duplicateKeys } = useMemo(() => {
    const base = parsed.map(buildPreviewRow);
    const keyFn = (r: PreviewRow) => (r.nome ? `nome:${String(r.nome).toLowerCase()}` : '');
  ...
  }, [parsed, mapping, dedupeStrategy, forceUppercase]);
  ...
  const previewSorted = useMemo(() => {
    return sortRows(
      ...
      [
        ...
        { id: 'setor', type: 'string', getValue: (r: PreviewRow) => r.setor ?? '' },
        { id: 'ativo', type: 'string', getValue: (r: PreviewRow) => (r.ativo ? 'Sim' : 'Não') },
  ...
  const handleImport = async () => {
    ...
    try {
      for (const row of preview) {
        ...
        try {
          await importFn(row.payload);
          ok += 1;
        } catch (e: any) {
          failed += 1;
          console.warn('[CSV_IMPORT][CARGOS] row failed', { line: row.line, error: e?.message || e });
        }
      }
      ...
      if (ok > 0) {
        addToast(`Importação concluída: ${ok} cargo(s) criado(s), ${failed} falha(s).`, 'success');
        onImported(summary);
      } else {
        addToast(`Nenhum cargo importado. ${failed} falha(s).`, 'warning');
      }
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Importar Cargos (CSV/XLSX)" size="4xl" bodyClassName="p-6 md:p-8">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (!canGoNextFromStep1) {
                    addToast('Mapeie o campo obrigatório "Nome do Cargo".', 'warning');
                    return;
                  }
                  setStep(2);
                }}
          ...
          <>
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm text-gray-600">
                Campos comuns: <code>nome</code>, <code>descricao</code>, <code>setor</code>, <code>responsabilidades</code>, <code>autoridades</code>, <code>ativo</code>.
              </div>
            ...
            <TextArea
              ...
              placeholder={'nome;setor;descricao;ativo\nAnalista de Qualidade;Qualidade;Responsável pelo SGQ;sim\nGerente Comercial;Comercial;Gestão de vendas;sim'}
              rows={12}
            />
          </>
        ) : null}
          ...
          <div className="rounded-xl border border-gray-200 bg-white/70 p-4 space-y-4">
            ...
            {duplicateKeys.length > 0 ? (
              <div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                Detectamos {duplicateKeys.length} cargo(s) com nome repetido no arquivo.
              </div>
            ) : null}
          </div>
        ) : null}
                  ...
                  <table className="min-w-full text-sm table-fixed">
                    ...
                    <thead className="sticky top-0 bg-gray-50 text-xs uppercase text-gray-500">
                      <tr>
                        ...
                        <ResizableSortableTh columnId="setor" label="Setor" sort={previewSort} onSort={(col) => setPreviewSort((prev) => toggleSort(prev as any, col))} onResizeStart={startPreviewResize} className="px-3 py-2" />
                        <ResizableSortableTh columnId="ativo" label="Ativo" sort={previewSort} onSort={(col) => setPreviewSort((prev) => toggleSort(prev as any, col))} onResizeStart={startPreviewResize} className="px-3 py-2" />
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button type="button" onClick={() => void handleImport()} disabled={isImporting || totals.valid === 0}>
                  ...
                  <span className="ml-2">Importar {totals.valid} cargo(s)</span>
                </Button>
              </div>
            </div>
          </>
