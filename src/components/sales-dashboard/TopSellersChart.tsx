...
const TopSellersChart = React.memo(function TopSellersChart(props: { items: Array<{ name: string; value: number }>; title?: string }) {
  const { items, title: titleText = 'Top 5 Vendedores' } = props;

  const option = useMemo(() => {
    const sellers = items.map((i) => i.name);
