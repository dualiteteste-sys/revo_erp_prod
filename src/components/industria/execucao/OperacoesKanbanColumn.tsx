import React from 'react';
import { Droppable } from '@hello-pangea/dnd';
import OperacoesKanbanCard from './OperacoesKanbanCard';
import { Operacao } from '@/services/industriaExecucao';

interface Props {
  columnId: string;
  title: string;
  items: Operacao[];
}

const OperacoesKanbanColumn: React.FC<Props> = ({ columnId, title, items }) => {
  return (
    <div className="flex flex-col w-72 bg-gray-100/80 rounded-xl flex-shrink-0 h-full border border-gray-200/50">
      <div className="p-3 border-b border-gray-200 flex justify-between items-center">
        <h3 className="font-semibold text-gray-700 text-sm">{title}</h3>
        <span className="bg-white px-2 py-0.5 rounded-full text-xs font-bold text-gray-500 shadow-sm">
            {items.length}
        </span>
      </div>
      <Droppable droppableId={columnId}>
        {(provided, snapshot) => (
          <div
            ref={provided.innerRef}
            {...provided.droppableProps}
            className={`flex-1 p-2 overflow-y-auto scrollbar-styled transition-colors ${snapshot.isDraggingOver ? 'bg-blue-50/50' : ''}`}
          >
            {items.map((item, index) => (
              <OperacoesKanbanCard key={item.id} item={item} index={index} />
            ))}
            {provided.placeholder}
          </div>
        )}
      </Droppable>
    </div>
  );
};

export default OperacoesKanbanColumn;
