...
export default function BeneficiamentoFormPanel({ ordemId, onSaveSuccess, onClose }: Props) {
  ...
  const loadDetails = async () => {
    try {
      const data = await getOrdemBeneficiamentoDetails(ordemId!);
      setFormData(data);
    } catch (e) {
      console.error(e);
  ...
  const handleAddEntrega = async (data: any) => {
    ...
    try {
      await manageEntregaBenef(
        formData.id,
        null,
        data.data_entrega!,
        data.quantidade_entregue!,
        data.status_faturamento!,
        data.documento_entrega, // Usando doc_ref como documento_entrega
        data.documento_faturamento,
        data.observacoes,
        'upsert'
      );
      await loadDetails();
        ...
        {activeTab === 'dados' && (
            <>
                ...
                <Section title="Controle" description="Rastreabilidade e Prazos.">
                    ...
                    <Input label="Doc. Interno" name="doc_ref" value={formData.documento_ref || ''} onChange={e => handleHeaderChange('documento_ref', e.target.value)} disabled={isLocked} className="sm:col-span-2" />
