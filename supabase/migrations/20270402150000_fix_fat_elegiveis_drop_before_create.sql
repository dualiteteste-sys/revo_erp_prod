-- Fix: PROD deploy fails with "cannot change return type of existing function"
-- because migration 20270401190000 uses CREATE OR REPLACE to add produto_sku
-- to the return type. PostgreSQL requires DROP + CREATE for return type changes.
-- This migration is idempotent: safe on dev (already has new signature) and prod.
