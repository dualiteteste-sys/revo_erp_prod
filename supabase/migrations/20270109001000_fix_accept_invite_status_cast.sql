/*
  Fix: RPC `public.accept_invite_for_current_user(p_empresa_id uuid)` retornando 400 (42804)

  Sintoma observado no app:
  - POST /rest/v1/rpc/accept_invite_for_current_user -> 400
  - details: "Returned type user_status_in_empresa does not match expected type text in column 3."

  Causa raiz:
  - `public.empresa_usuarios.status` é um ENUM (ex.: user_status_in_empresa).
  - A função declara `returns table (..., status text)`, mas o `return query` devolve o enum sem cast.

  Solução:
  - Cast explícito de `eu.status::text` no `return query`.
*/
...
begin
  ...
  return query
  select eu.empresa_id, eu.user_id, (eu.status::text) as status
  from public.empresa_usuarios eu
  where eu.empresa_id = p_empresa_id
    and eu.user_id = v_user_id;
end;
