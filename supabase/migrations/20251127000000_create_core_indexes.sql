-- Migration: Create Core Indexes (v13)
-- Description: Creates essential indexes for RLS (empresa_id) and Foreign Keys to optimize performance and prevent locking.
...
----------------------------
-- 2) Índices FK-leading (1ª coluna da FK)
----------------------------
-- compras / contas / CRM
...
-- indústria (beneficiamento / produção / ordens / roteiros / operações)
...
-- pessoas / produtos / profiles / RH / permissões
...
CREATE INDEX IF NOT EXISTS rh_cargo_competencias_competencia_id_idx          ON public.rh_cargo_competencias(competencia_id);
CREATE INDEX IF NOT EXISTS rh_colaborador_competencias_competencia_id_idx    ON public.rh_colaborador_competencias(competencia_id);
CREATE INDEX IF NOT EXISTS rh_colaboradores_cargo_id_idx                     ON public.rh_colaboradores(cargo_id);
CREATE INDEX IF NOT EXISTS rh_colaboradores_user_id_idx                      ON public.rh_colaboradores(user_id);
CREATE INDEX IF NOT EXISTS role_permissions_role_id_idx                      ON public.role_permissions(role_id);

-- subscriptions / user_active_empresa / user_permission_overrides / vendas
