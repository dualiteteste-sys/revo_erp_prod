  ...
  loop
    ...
    begin
      ...
      if v_aliq is null then
        v_aliq := 0;
        -- FIX: replaced format() with concatenation to avoid "unrecognized format() type specifier '.'" error
        v_warn := array_append(v_warn, 'ICMS: alíquota não configurada (' || upper(coalesce(v_emitente.endereco_uf,'')) || '→' || upper(coalesce(v_dest_end.uf,'')) || '). Assumindo 0%.');
      end if;
