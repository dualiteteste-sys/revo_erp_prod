-- Ensure industria_operador_upsert calls pg_catalog.gen_salt (available via pgcrypto)
begin;
...
begin
  ...
  if v_id is null then
    insert into public.industria_operadores (
      ...
      p_nome,
      p_email,
      crypt(p_pin, pg_catalog.gen_salt('bf'::text)),
    ...
    update public.industria_operadores
       set nome = coalesce(p_nome, nome),
           ...
           pin_hash = case when p_pin is not null then crypt(p_pin, pg_catalog.gen_salt('bf'::text)) else pin_hash end
     where id = v_id
