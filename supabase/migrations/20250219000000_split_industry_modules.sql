...
do $$
begin
  if not exists (select 1 from pg_trigger where tgname = 'handle_updated_at_ind_prod_ordens') then
    create trigger handle_updated_at_ind_prod_ordens before update on public.industria_producao_ordens for each row execute procedure public.tg_set_updated_at();
  end if;
  if not exists (select 1 from pg_trigger where tgname = 'handle_updated_at_ind_prod_comp') then
    create trigger handle_updated_at_ind_prod_comp before update on public.industria_producao_componentes for each row execute procedure public.tg_set_updated_at();
  end if;
  if not exists (select 1 from pg_trigger where tgname = 'handle_updated_at_ind_prod_entregas') then
    create trigger handle_updated_at_ind_prod_entregas before update on public.industria_producao_entregas for each row execute procedure public.tg_set_updated_at();
  end if;
end $$;
...
create policy "ind_prod_ordens_all" on public.industria_producao_ordens for all using (empresa_id = public.current_empresa_id()) with check (empresa_id = public.current_empresa_id());
create policy "ind_prod_comp_all" on public.industria_producao_componentes for all using (empresa_id = public.current_empresa_id()) with check (empresa_id = public.current_empresa_id());
create policy "ind_prod_entregas_all" on public.industria_producao_entregas for all using (empresa_id = public.current_empresa_id()) with check (empresa_id = public.current_empresa_id());
...
begin
  ...
  join public.produtos p on o.produto_final_id = p.id
  left join public.industria_producao_entregas e on e.ordem_id = o.id
  where o.empresa_id = v_empresa_id
    and (p_search is null or o.numero::text ilike '%' || p_search || '%' or p.nome ilike '%' || p_search || '%')
...
declare
  ...
  v_entregas jsonb;
begin
  select to_jsonb(o.*) || jsonb_build_object('produto_nome', p.nome)
  into v_ordem
  from public.industria_producao_ordens o
  join public.produtos p on o.produto_final_id = p.id
  where o.id = p_id and o.empresa_id = v_empresa_id;

  if v_ordem is null then return null; end if;

  select jsonb_agg(to_jsonb(c.*) || jsonb_build_object('produto_nome', p.nome))
  into v_componentes
  from public.industria_producao_componentes c
  join public.produtos p on c.produto_id = p.id
  where c.ordem_id = p_id and c.empresa_id = v_empresa_id;

  select jsonb_agg(to_jsonb(e.*) order by e.data_entrega desc)
  into v_entregas
  from public.industria_producao_entregas e
  where e.ordem_id = p_id and e.empresa_id = v_empresa_id;

  return v_ordem || jsonb_build_object(
...
begin
  if p_payload->>'id' is not null then
    update public.industria_producao_ordens set
    ...
    insert into public.industria_producao_ordens (
      empresa_id, origem_ordem, produto_final_id, quantidade_planejada, unidade,
      status, prioridade, data_prevista_inicio, data_prevista_fim, data_prevista_entrega,
...
declare
  v_empresa_id uuid := public.current_empresa_id();
begin
  if not exists (select 1 from public.industria_producao_ordens where id = p_ordem_id and empresa_id = v_empresa_id) then
    raise exception 'Ordem não encontrada.';
  end if;

  if p_action = 'delete' then
    delete from public.industria_producao_componentes where id = p_componente_id and empresa_id = v_empresa_id;
  else
    if p_componente_id is not null then
      update public.industria_producao_componentes set
        produto_id = p_produto_id,
        quantidade_planejada = p_quantidade_planejada,
        unidade = p_unidade
      where id = p_componente_id and empresa_id = v_empresa_id;
    else
      insert into public.industria_producao_componentes (empresa_id, ordem_id, produto_id, quantidade_planejada, unidade)
      values (v_empresa_id, p_ordem_id, p_produto_id, p_quantidade_planejada, p_unidade);
    end if;
  end if;
end;
$$;
...
begin
  select quantidade_planejada into v_qtd_planejada from public.industria_producao_ordens where id = p_ordem_id and empresa_id = v_empresa_id;
  if v_qtd_planejada is null then raise exception 'Ordem não encontrada.'; end if;

  if p_action = 'delete' then
    delete from public.industria_producao_entregas where id = p_entrega_id and empresa_id = v_empresa_id;
  else
    select coalesce(sum(quantidade_entregue), 0) into v_total_entregue
    from public.industria_producao_entregas
    where ordem_id = p_ordem_id and empresa_id = v_empresa_id and (p_entrega_id is null or id <> p_entrega_id);
    ...
    if p_entrega_id is not null then
      update public.industria_producao_entregas set
        data_entrega = p_data_entrega,
        quantidade_entregue = p_quantidade_entregue,
        documento_ref = p_documento_ref,
        observacoes = p_observacoes
      where id = p_entrega_id and empresa_id = v_empresa_id;
    else
      insert into public.industria_producao_entregas (empresa_id, ordem_id, data_entrega, quantidade_entregue, documento_ref, observacoes)
      values (v_empresa_id, p_ordem_id, p_data_entrega, p_quantidade_entregue, p_documento_ref, p_observacoes);
    end if;
  end if;
...
create or replace function public.industria_producao_update_status(p_id uuid, p_status text, p_prioridade int)
returns void
language plpgsql security definer set search_path = pg_catalog, public
as $$
begin
  update public.industria_producao_ordens set status = p_status, prioridade = p_prioridade
...
do $$
begin
  if not exists (select 1 from pg_trigger where tgname = 'handle_updated_at_ind_benef_ordens') then
    create trigger handle_updated_at_ind_benef_ordens before update on public.industria_benef_ordens for each row execute procedure public.tg_set_updated_at();
  end if;
  if not exists (select 1 from pg_trigger where tgname = 'handle_updated_at_ind_benef_comp') then
    create trigger handle_updated_at_ind_benef_comp before update on public.industria_benef_componentes for each row execute procedure public.tg_set_updated_at();
  end if;
  if not exists (select 1 from pg_trigger where tgname = 'handle_updated_at_ind_benef_entregas') then
    create trigger handle_updated_at_ind_benef_entregas before update on public.industria_benef_entregas for each row execute procedure public.tg_set_updated_at();
  end if;
end $$;
...
create policy "ind_benef_ordens_all" on public.industria_benef_ordens for all using (empresa_id = public.current_empresa_id()) with check (empresa_id = public.current_empresa_id());
create policy "ind_benef_comp_all" on public.industria_benef_componentes for all using (empresa_id = public.current_empresa_id()) with check (empresa_id = public.current_empresa_id());
create policy "ind_benef_entregas_all" on public.industria_benef_entregas for all using (empresa_id = public.current_empresa_id()) with check (empresa_id = public.current_empresa_id());
...
begin
  ...
  left join public.industria_benef_entregas e on e.ordem_id = o.id
  where o.empresa_id = v_empresa_id
...
begin
  ...
  select jsonb_agg(to_jsonb(c.*) || jsonb_build_object('produto_nome', p.nome))
  into v_componentes
  from public.industria_benef_componentes c
  join public.produtos p on c.produto_id = p.id
  where c.ordem_id = p_id and c.empresa_id = v_empresa_id;
...
begin
  if p_payload->>'id' is not null then
    update public.industria_benef_ordens set
    ...
    insert into public.industria_benef_ordens (
      empresa_id, cliente_id, produto_servico_id, produto_material_cliente_id, usa_material_cliente,
      quantidade_planejada, unidade, status, prioridade, data_prevista_entrega,
      pedido_cliente_ref, lote_cliente, documento_ref, observacoes
    ) values (
      v_empresa_id,
...
declare
  v_empresa_id uuid := public.current_empresa_id();
begin
  if not exists (select 1 from public.industria_benef_ordens where id = p_ordem_id and empresa_id = v_empresa_id) then
    raise exception 'Ordem não encontrada.';
  end if;

  if p_action = 'delete' then
    delete from public.industria_benef_componentes where id = p_componente_id and empresa_id = v_empresa_id;
  else
    if p_componente_id is not null then
      update public.industria_benef_componentes set
        produto_id = p_produto_id,
        quantidade_planejada = p_quantidade_planejada,
        unidade = p_unidade
      where id = p_componente_id and empresa_id = v_empresa_id;
    else
      insert into public.industria_benef_componentes (empresa_id, ordem_id, produto_id, quantidade_planejada, unidade)
      values (v_empresa_id, p_ordem_id, p_produto_id, p_quantidade_planejada, p_unidade);
    end if;
  end if;
end;
...
declare
  ...
  v_total_entregue numeric;
begin
  select quantidade_planejada into v_qtd_planejada from public.industria_benef_ordens where id = p_ordem_id and empresa_id = v_empresa_id;
  if v_qtd_planejada is null then raise exception 'Ordem não encontrada.'; end if;

  if p_action = 'delete' then
    delete from public.industria_benef_entregas where id = p_entrega_id and empresa_id = v_empresa_id;
  else
    select coalesce(sum(quantidade_entregue), 0) into v_total_entregue
    ...
    if p_entrega_id is not null then
      update public.industria_benef_entregas set
        data_entrega = p_data_entrega,
        quantidade_entregue = p_quantidade_entregue,
      ...
      insert into public.industria_benef_entregas (
        empresa_id, ordem_id, data_entrega, quantidade_entregue, status_faturamento,
        documento_entrega, documento_faturamento, observacoes
      )
      values (
        v_empresa_id, p_ordem_id, p_data_entrega, p_quantidade_entregue, p_status_faturamento,
        p_documento_entrega, p_documento_faturamento, p_observacoes
      );
    end if;
  end if;
...
-- Update Status (Kanban)
create or replace function public.industria_benef_update_status(p_id uuid, p_status text, p_prioridade int)
returns void
language plpgsql security definer set search_path = pg_catalog, public
as $$
begin
  update public.industria_benef_ordens set status = p_status, prioridade = p_prioridade
  where id = p_id and empresa_id = public.current_empresa_id();
...
-- =============================================
-- 3. DASHBOARD UNIFICADO (Opcional)
-- =============================================
...
begin
  select jsonb_agg(t) into v_prod_status from (
    select status, count(*) as total from public.industria_producao_ordens where empresa_id = v_empresa_id group by status
  ) t;

  select jsonb_agg(t) into v_benef_status from (
    select status, count(*) as total from public.industria_benef_ordens where empresa_id = v_empresa_id group by status
  ) t;

  select count(*) into v_total_prod from public.industria_producao_ordens where empresa_id = v_empresa_id;
  select count(*) into v_total_benef from public.industria_benef_ordens where empresa_id = v_empresa_id;
