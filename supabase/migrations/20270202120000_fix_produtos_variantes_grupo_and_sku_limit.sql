-- Fix (Produtos > Variações):
-- 1) Variantes devem herdar `grupo_id` do produto pai.
-- 2) Remover limitação prática de 99 tentativas na geração de SKU alternativo (suporta muitas variações).
...
begin
  ...
  foreach v_val in array p_valores_text loop
    ...
    if v_base_sku is not null then
      ...
      for v_try in 0..v_max_tries loop
        if v_try = 0 then
          v_candidate_sku := v_base_sku;
        else
        ...
        if not found then
          v_existing_id := null;
          v_existing_parent_id := null;
          v_sku_ok := true; -- SKU livre
          exit;
        end if;

        if v_existing_parent_id = v_parent.id then
          v_sku_ok := true; -- SKU pertence a uma variante do mesmo pai (idempotente)
          exit;
        end if;
      end loop;

      if not v_sku_ok then
        raise exception 'Não foi possível gerar SKU único para "%" (tentativas=%).', v_val, v_max_tries;
      end if;
    end if;
