-- ============================================================================
-- Serviços / Contratos (MVP3): Gerar títulos até data futura (p_until)
-- ============================================================================
--
-- Objetivo:
-- - Permitir gerar Contas a Receber não só “até hoje”, mas até uma data futura,
--   sem depender de o schedule já existir (auto gera schedule até cobrir p_until).
-- - Mantém idempotência via origem_tipo/origem_id (schedule.id).

do $$
begin
  -- Remove overload antigo para evitar ambiguidade no PostgREST (/rpc).
  execute 'drop function if exists public.servicos_contratos_billing_generate_receivables(uuid, date)';
end $$;
...
begin
  ...
  -- Garante schedule suficiente para cobrir p_until (permite datas futuras).
  ...
  if v_auto_months < 1 then
    v_auto_months := 1;
  end if;
  if v_auto_months > 36 then
    v_auto_months := 36;
  end if;
  ...
  return jsonb_build_object('ok', true, 'created', v_created, 'months_ahead', v_auto_months, 'until', coalesce(p_until, current_date));
end;
$$;
