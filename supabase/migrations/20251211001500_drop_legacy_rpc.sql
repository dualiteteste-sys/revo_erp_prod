-- Drop legacy RPC overload to fix "Tipo de evento inválido"
-- The old function had 3 arguments and expected 'inicio_producao'
-- The new function has 2 arguments and expects 'iniciar'

BEGIN;

-- Drop the specific 3-argument signature from operations_full.sql
DROP FUNCTION IF EXISTS public.industria_producao_registrar_evento(uuid, text, text);

-- Also drop the 2-argument version IF it has the old parameter name 'p_tipo' just in case
-- (Though Postgres matches by types, parameter names matter for Supabase RPC)
-- We'll just define the CORRECT one again to be 100% sure.
