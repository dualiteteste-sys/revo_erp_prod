-- Add grupo_id to create_product_for_current_user and update_product_for_current_user
-- This fixes the issue where the selected product group was not being saved.

-- 1. Update create_product_for_current_user
...
begin
  ...
  values (
    ...
    nullif(payload->>'grupo_id','')::uuid, -- Added grupo_id
...
-- 2. Update update_product_for_current_user
...
begin
  ...
  set
    ...
    grupo_id             = case when patch ? 'grupo_id' then nullif(patch->>'grupo_id','')::uuid else grupo_id end, -- Added grupo_id
