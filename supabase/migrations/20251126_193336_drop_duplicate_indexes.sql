-- Migration: Drop duplicate/unused btree indexes (generated on 2025-11-26, America/Sao_Paulo)
-- Notes:
-- - Do NOT run inside an explicit transaction: DROP INDEX CONCURRENTLY requires autocommit.
-- - Safe/idempotent: uses IF EXISTS; primary/unique/constraint-backed indexes were NOT included.
-- - You can re-create later with CREATE INDEX CONCURRENTLY if needed.
-- - Suggested session settings:
--     SET lock_timeout = '5s';
--     SET statement_timeout = '10min';
-- ---------------------------------------------------------------

DROP INDEX CONCURRENTLY IF EXISTS auth.users_instance_id_email_idx;
...
DROP INDEX CONCURRENTLY IF EXISTS public.idx_fk_industria_ordens_ent_empresa_id_0fc9b6;
DROP INDEX CONCURRENTLY IF EXISTS public.idx_fk_industria_ordens_ent_ordem_id_dfb6ce;
