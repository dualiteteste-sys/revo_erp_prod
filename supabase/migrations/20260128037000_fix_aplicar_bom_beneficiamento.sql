-- =============================================================================
-- Fix: aplicar BOM em Ordem de Beneficiamento usando o modelo unificado (industria_ordens)
-- =============================================================================
BEGIN;
...
DECLARE
  ...
  v_bom_desc            text;
BEGIN
  SELECT b.produto_final_id, b.codigo, b.descricao
    INTO v_produto_bom, v_bom_codigo, v_bom_desc
  ...
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'industria_ordens'
      AND column_name = 'bom_aplicado_id'
  ) THEN
    UPDATE public.industria_ordens
       SET bom_aplicado_id = p_bom_id,
           bom_aplicado_desc = coalesce(v_bom_codigo, v_bom_desc, bom_aplicado_desc)
     WHERE id = p_ordem_id
