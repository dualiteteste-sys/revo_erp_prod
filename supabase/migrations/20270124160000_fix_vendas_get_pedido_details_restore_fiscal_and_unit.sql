-- Fix: vendas_get_pedido_details must include fiscal fields (NCM/CFOP/CST/CSOSN)
-- and also include product unit/SKU for UI (venda por peso) without regressing
-- the LEFT JOIN resiliency added previously.
...
declare
  ...
  v_itens   jsonb;
begin
  -- cabeçalho (LEFT JOIN: não derrubar o pedido quando o cliente estiver ausente)
  select
    to_jsonb(p.*)
