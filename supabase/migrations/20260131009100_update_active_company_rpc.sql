/*
  RPC: update_active_company(p_patch jsonb) -> jsonb

  Problema em PROD após reset:
  - CompanySettingsForm salva via RPC `update_active_company`
  - Função existia apenas em migrations_legacy, então em banco limpo ficou ausente (404).

  Objetivo:
  - Atualizar a empresa ativa do usuário atual (public.current_empresa_id())
  - Ser tolerante a variações de schema (checa existência de colunas)
*/
