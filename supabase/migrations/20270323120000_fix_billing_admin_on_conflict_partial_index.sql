-- Fix: billing admin function ON CONFLICT must match partial unique index
--
-- Bug: servicos_contratos_billing_generate_schedule_admin uses
--   ON CONFLICT (empresa_id, contrato_id, kind, competencia) DO UPDATE ...
-- but the unique index is partial:
...
--
-- The non-admin version was fixed in 20270111193000, but the admin version was missed.
-- This applies the same fix to the admin function.
