/*
  ...
  - Tabela principal de cobranças ligadas a Contas a Receber e Tesouraria.
  - Tabela de eventos de cobrança (status, webhooks, erros).
  - RLS por operação em todas as tabelas.
  - RPCs de listagem, detalhes, upsert, delete seguro e resumo.
  - FKs e validações para financeiro_contas_receber criadas/ativadas apenas se a tabela existir.

  ## Impact Summary
  - Segurança:
    - Multi-tenant por empresa_id + RLS em todas as tabelas.
    - RPCs com SECURITY DEFINER e search_path = pg_catalog, public.
    - Uso consistente de public.current_empresa_id().
  - Compatibilidade:
    - create table/index if not exists.
    - FKs condicionais para financeiro_contas_receber, evitando erro em bancos onde o módulo ainda não existe.
  - Reversibilidade:
    - Tabelas, índices, policies e funções podem ser dropados em migração futura.
  - Performance:
    - Índices em empresa_id, status, datas e relacionamentos.
    - Listagens com paginação + total_count via window function.
*/
...
--    (sem FK direta para financeiro_contas_receber; FK virá em bloco condicional)
-- =============================================
...
create table if not exists public.financeiro_cobrancas_bancarias (
  ...
  conta_receber_id      uuid,                        -- FK opcional para título (contas a receber)
  ...
  status                text not null default 'pendente_emissao'
                        check (status in (
                          'pendente_emissao', -- criada no sistema, ainda não enviada/registrada
                          'emitida',          -- gerado boleto/QR/local
                          'registrada',       -- registrada no banco
                          'enviada',          -- enviada ao cliente
                          'liquidada',        -- paga e conciliada
                          'baixada',          -- baixa manual/automática sem pagamento integral
                          'cancelada',        -- cancelada
                          'erro'              -- erro de registro/envio
                        )),
...
create table if not exists public.financeiro_cobrancas_bancarias_eventos (
  ...
  status_novo      text,
  mensagem         text,             -- mensagem amigável
  detalhe_tecnico  text,             -- JSON/raw de retorno, stacktrace, payload etc.
...
-- 4.1) Listar cobranças (paginado)
create or replace function public.financeiro_cobrancas_bancarias_list(
  p_q              text   default null,
  p_status         text   default null,   -- se null, traz todos
...
-- 4.2) Detalhes da cobrança (jsonb)
--      Sem join estático com financeiro_contas_receber; busca do título é dinâmica e tolerante
...
-- 4.3) Upsert de cobrança (jsonb)
--      Validação de contas a receber feita via SQL dinâmico tolerante à ausência da tabela
...
-- 4.4) Delete seguro (bloqueia liquidadas/baixadas)
...
-- 4.5) Resumo de cobranças (cards/dashboard)
create or replace function public.financeiro_cobrancas_bancarias_summary(
  p_start_venc date default null,
  p_end_venc   date default null,
  p_status     text default null  -- opcional: filtrar subset de status
)
returns jsonb
language plpgsql
security definer
