-- Fix: tipo_embalagem NOT NULL causa falha em create_product e CSV import
-- Bug P1: 23 ocorrências em prod (2026-03-20)
-- Root cause: coluna definida como `tipo_embalagem not null default 'pacote_caixa'`,
--   mas a RPC usa nullif(payload->>'tipo_embalagem','')::tipo_embalagem que retorna NULL
--   quando o payload envia null (ex: import CSV/XLSX), sobrescrevendo o DEFAULT da coluna.
-- Fix: COALESCE na RPC create_product para fallback 'pacote_caixa'.
...
BEGIN
  ...
  VALUES (
    ...
    payload->>'ncm',
    payload->>'cest',
    COALESCE(nullif(payload->>'tipo_embalagem','')::public.tipo_embalagem, 'pacote_caixa'::public.tipo_embalagem),
    payload->>'embalagem',
