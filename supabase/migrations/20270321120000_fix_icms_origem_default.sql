-- Fix: icms_origem NOT NULL sem DEFAULT causa falha em create_product e CSV import
-- Bug P1: 70 ocorrências em prod (2026-03-19)
-- Root cause: coluna definida como `smallint not null` sem DEFAULT,
--   e a RPC usa nullif(payload->>'icms_origem','')::integer que retorna NULL
--   quando o payload não contém o campo (ex: import CSV/XLSX).
-- Fix: (1) ADD DEFAULT 0 na coluna, (2) COALESCE na RPC create_product.

-- 1) Adicionar DEFAULT 0 à coluna icms_origem
ALTER TABLE public.produtos
  ALTER COLUMN icms_origem SET DEFAULT 0;

-- 2) Reescrever create_product_for_current_user com COALESCE no icms_origem
