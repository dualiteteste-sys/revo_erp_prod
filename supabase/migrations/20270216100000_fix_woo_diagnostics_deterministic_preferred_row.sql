...
/*
  Problema:
  - Ambientes com múltiplas linhas em `public.ecommerces` para provider='woo' podiam
    retornar diagnóstico inconsistente, pois a função anterior usava LIMIT 1 sem ORDER BY.
  - Isso gerava oscilação visual no front (credenciais "salvas" -> "não salvas").

  Correção:
  - Tornar a seleção determinística com prioridade por:
      1) conexão Woo com credenciais presentes + status connected
      2) conexão Woo com credenciais presentes
      3) status connected/pending/error
      4) updated_at DESC, created_at DESC
  - Preservar contrato atual de retorno da RPC.
*/
