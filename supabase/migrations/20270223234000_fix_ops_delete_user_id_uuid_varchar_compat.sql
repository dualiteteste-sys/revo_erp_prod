...
BEGIN
  ...
  END IF;
  v_empresa_id_text := v_empresa_id::text;

  SELECT e.nome, e.slug
    INTO v_empresa_nome, v_empresa_slug
  FROM public.empresas e
  WHERE e.id::text = v_empresa_id_text;

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Empresa ativa não encontrada para exclusão.' USING errcode = 'P0002';
  END IF;

  CREATE TEMP TABLE tmp_ops_delete_member_users(user_id text PRIMARY KEY) ON COMMIT DROP;
  INSERT INTO tmp_ops_delete_member_users(user_id)
  SELECT DISTINCT eu.user_id::text
  FROM public.empresa_usuarios eu
  WHERE eu.empresa_id::text = v_empresa_id_text;

  CREATE TEMP TABLE tmp_ops_delete_auth_users(user_id text PRIMARY KEY) ON COMMIT DROP;
  ...
  WHERE NOT EXISTS (
    SELECT 1
    FROM public.empresa_usuarios eu
    WHERE eu.user_id::text = mu.user_id
      AND eu.empresa_id::text <> v_empresa_id_text
  );
  ...
  INTO v_users
  FROM tmp_ops_delete_member_users mu
  LEFT JOIN auth.users au ON au.id::text = mu.user_id;
  ...
  FOR r IN
    ...
    ORDER BY c.relname
  LOOP
    EXECUTE format('SELECT count(*)::bigint FROM public.%I WHERE empresa_id::text = $1', r.table_name)
      INTO v_rows
      USING v_empresa_id_text;
  ...
  RETURN jsonb_build_object(
    ...
    'required_confirmation', format('EXCLUIR %s', v_empresa_id_text),
    'users', v_users,
...
BEGIN
  ...
  v_expected_confirmation := format('EXCLUIR %s', v_empresa_id_text);
  ...
  LOOP
    ...
    LOOP
      BEGIN
        EXECUTE format('DELETE FROM public.%I WHERE empresa_id::text = $1', r.table_name)
          USING v_empresa_id_text;
  ...
  BEGIN
    ...
    WHERE (storage.foldername(o.name))[1] = v_empresa_id_text;
  EXCEPTION
    WHEN OTHERS THEN
      v_storage_objects_pending := 0;
  END;

  IF v_storage_objects_pending > 0 THEN
    v_storage_cleanup_status := 'pending_storage_api_cleanup';
  ELSE
    v_storage_cleanup_status := 'storage_api_clean';
  END IF;
  ...
  IF to_regclass('public.profiles') IS NOT NULL THEN
    EXECUTE 'DELETE FROM public.profiles p WHERE p.id::text = ANY(SELECT user_id FROM tmp_ops_delete_auth_users)';
    GET DIAGNOSTICS v_deleted_profiles_rows = ROW_COUNT;
  END IF;

  IF to_regclass('auth.identities') IS NOT NULL THEN
    EXECUTE 'DELETE FROM auth.identities i WHERE i.user_id::text = ANY(SELECT user_id FROM tmp_ops_delete_auth_users)';
    GET DIAGNOSTICS v_deleted_identities_rows = ROW_COUNT;
  END IF;

  IF to_regclass('auth.sessions') IS NOT NULL THEN
    EXECUTE 'DELETE FROM auth.sessions s WHERE s.user_id::text = ANY(SELECT user_id FROM tmp_ops_delete_auth_users)';
    GET DIAGNOSTICS v_deleted_sessions_rows = ROW_COUNT;
  END IF;

  IF to_regclass('auth.refresh_tokens') IS NOT NULL THEN
    EXECUTE 'DELETE FROM auth.refresh_tokens rt WHERE rt.user_id::text = ANY(SELECT user_id FROM tmp_ops_delete_auth_users)';
    GET DIAGNOSTICS v_deleted_refresh_tokens_rows = ROW_COUNT;
  END IF;

  EXECUTE 'DELETE FROM auth.users u WHERE u.id::text = ANY(SELECT user_id FROM tmp_ops_delete_auth_users)';
