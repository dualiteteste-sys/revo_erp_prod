-- ============================================================================
-- Onboarding “Gate Suave”: status consolidado via RPC (evita múltiplos HEAD/GET)
-- ============================================================================

-- Retorna os checks do onboarding para a empresa ativa do usuário atual.
-- Observação: security definer para evitar fricção por RLS em checks de leitura.
drop function if exists public.onboarding_checks_for_current_empresa();
create function public.onboarding_checks_for_current_empresa()
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  -- Centros de custo (mínimo: existir pelo menos 1)
  select exists(select 1 from public.centros_de_custo c where c.empresa_id = v_empresa_id)
    into v_has_centros;
  ...
  v_checks := jsonb_build_array(
    ...
    jsonb_build_object(
      ...
      'description', case when v_has_centros
        then 'Ok: já existe pelo menos 1 centro de custo.'
        else 'Cadastre centros de custo para relatórios e auditoria.'
      end,
      'status', case when v_has_centros then 'ok' else 'warn' end,
