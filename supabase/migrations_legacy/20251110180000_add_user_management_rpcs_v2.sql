...
-- 2) LIST v2 com casts explícitos para bater o RETURNS TABLE
...
-- Fallback de enum: tenta 'SUSPENDED'; se não existir, usa 'INACTIVE'
...
begin
  ...
  if v_pending_label is null then
    raise exception 'ENUM_LABEL_MISSING: add PENDING (or INVITED) to user_status_in_empresa';
  end if;

  delete from public.empresa_usuarios
   where empresa_id = public.current_empresa_id()
     and user_id = p_user_id
     and status = v_pending_label::public.user_status_in_empresa;
end;
$$;
