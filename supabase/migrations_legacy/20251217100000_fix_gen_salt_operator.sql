-- Recria industria_operador_upsert usando gen_salt do pgcrypto de forma explícita
begin;
...
begin
  ...
  if v_id is null then
    insert into public.industria_operadores (
      ...
      p_nome,
      p_email,
      crypt(p_pin, public.gen_salt('bf'::text)),
    ...
    update public.industria_operadores
       set nome = coalesce(p_nome, nome),
           ...
           pin_hash = case when p_pin is not null then crypt(p_pin, public.gen_salt('bf'::text)) else pin_hash end
     where id = v_id
