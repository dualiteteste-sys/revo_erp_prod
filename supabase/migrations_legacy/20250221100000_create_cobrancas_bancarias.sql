/*
  # Financeiro - Cobranças Bancárias (backend completo)
  ...
  - Compatibilidade:
    ...
    - drop function if exists antes de recriar.
    - Não altera estruturas existentes de Contas a Receber/Tesouraria.
  - Reversibilidade:
...
create table if not exists public.financeiro_cobrancas_bancarias (
  ...
  constraint fin_cobr_cr_fkey
    foreign key (conta_receber_id) references public.financeiro_contas_receber(id)
      on delete set null,
...
begin
  ...
  select
    ...
    || jsonb_build_object(
         ...
         'conta_nome',   cc.nome,
         'titulo_numero', cr.documento_ref,
  ...
  left join public.financeiro_contas_correntes cc
    on cc.id = c.conta_corrente_id
   and cc.empresa_id = v_empresa
  left join public.financeiro_contas_receber cr
    on cr.id = c.conta_receber_id
  where c.id = p_id
