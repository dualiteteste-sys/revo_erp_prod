-- [RPC FIX] create_update_partner — versão sem JOIN em empresa_usuarios; sem is_principal
...
begin
  ...
  -- ID da pessoa vindo do payload (quando edição)
  ...
  if v_pessoa_id is null then
    insert into public.pessoas (
      ...
      inscr_municipal,
      observacoes,
      -- campos complementares (existentes no schema do projeto)
      tipo_pessoa,       -- public.tipo_pessoa_enum
      fantasia,
      codigo_externo,
      contribuinte_icms, -- public.contribuinte_icms_enum
      contato_tags       -- text[]
    )
    values (
      v_empresa_id,
      ...
      case when v_pessoa_payload ? 'isento_ie'
           then (v_pessoa_payload->>'isento_ie')::boolean
           else null end,
      ...
      case
        when jsonb_typeof(v_pessoa_payload->'contato_tags') = 'array'
          then (select array_agg(elem) from jsonb_array_elements_text(v_pessoa_payload->'contato_tags') as elem)
        else null
      end
    )
  ...
  -- ENDEREÇOS: upsert simples pelos IDs presentes
  if jsonb_typeof(v_enderecos_payload) = 'array' then
  ...
  -- CONTATOS: upsert simples pelos IDs presentes
  if jsonb_typeof(v_contatos_payload) = 'array' then
  ...
  -- Retorna o objeto completo esperado pelo frontend
  return public.get_partner_details(v_pessoa_id);
end;
$$;

-- Força o PostgREST a recarregar o cache
