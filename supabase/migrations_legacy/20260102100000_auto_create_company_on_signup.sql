/*
  ...
  - REVOKE/GRANT restritos; logs leves.
...
-- 1) Função de sistema: cria e/ou ativa empresa para um user explícito (sem JWT)
  ...
  if v_has_membership then
    ...
    if not exists (select 1 from public.user_active_empresa uae where uae.user_id = p_user_id) then
      ...
      if v_empresa_id is not null then
        ...
        if not found then
          insert into public.user_active_empresa(user_id, empresa_id) values (p_user_id, v_empresa_id);
        end if;
      end if;
    end if;
    return;
...
grant execute on function public.system_bootstrap_empresa_for_user(uuid, text, text) to authenticated, service_role;

-- 2) Função TRIGGER: usa NEW.id (auth.users)
