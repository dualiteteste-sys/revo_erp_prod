...
create table if not exists public.industria_materiais_cliente (
  ...
  cliente_id      uuid not null,                 -- pessoas.id (tipo cliente)
  produto_id      uuid not null,                 -- produtos.id (item do catálogo interno)
  codigo_cliente  text,                          -- código que o cliente usa (opcional)
  nome_cliente    text,                          -- nome/descrição como no cliente (opcional)
  ...
  constraint ind_matcli_empresa_cliente_produto_codigo_uk
    unique (empresa_id, cliente_id, produto_id, codigo_cliente)
);
...
-- 3.1) Listagem (auto-suficiente para autocomplete do campo)
create or replace function public.industria_materiais_cliente_list(
  p_cliente_id uuid   default null,   -- se null, lista todos os clientes da empresa
  p_search     text   default null,   -- busca por codigo_cliente/nome_cliente/produto.nome
  p_ativo      boolean default true,  -- default só ativos
